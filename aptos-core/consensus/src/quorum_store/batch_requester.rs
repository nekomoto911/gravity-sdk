...
use crate::{
    ...
    quorum_store::types::{BatchKey, BatchRequest, BatchResponse, PersistedValue},
};
...
use gaptos::{
    ...
    aptos_types::{transaction::SignedTransaction, validator_verifier::ValidatorVerifier, PeerId},
};
use rand::Rng;
...
impl BatchRequesterState {
    fn new(
        ...
        Self { signers, next_index: 0, ret_tx, num_retries: 0, retry_limit }
    }
    ...
    fn next_request_peers(&mut self, num_peers: usize) -> Option<Vec<PeerId>> {
        if self.num_retries == 0 {
            ...
            counters::SENT_BATCH_REQUEST_COUNT.inc_by(num_peers.min(self.signers.len()) as u64);
        } else {
            counters::SENT_BATCH_REQUEST_RETRY_COUNT
                .inc_by(num_peers.min(self.signers.len()) as u64);
        }
    ...
    fn serve_request(self, digest: HashValue, maybe_payload: Option<Vec<SignedTransaction>>) {
        if let Some(payload) = maybe_payload {
            trace!("QS: batch to oneshot, digest {}, tx {:?}", digest, self.ret_tx);
            if self.ret_tx.send(Ok(payload)).is_err() {
                debug!("Receiver of requested batch not available for digest {}", digest)
            };
        } else if self.ret_tx.send(Err(ExecutorError::CouldNotGetData)).is_err() {
            debug!("Receiver of requested batch not available for unavailable digest {}", digest);
        }
    }
}
