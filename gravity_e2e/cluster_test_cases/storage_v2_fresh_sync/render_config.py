...
storage_v2_upgrade's renderer extended with a SECOND binary source: the
tracked templates carry placeholders, the untracked test_params.toml
supplies

- [source]            -> {{SOURCE}} on the four legacy nodes (the OLD,
...
Self-contained on purpose (no imports beyond stdlib):
...
def resolve_hardfork_value(value, now=None):
    ...
    becomes ``now + sum(components)`` (unix seconds). Anything else raises."""
...
def source_to_inline_toml(source: dict) -> str:
    """Render a [source]/[sf_source] table as a TOML inline table string."""
...
def validate_sf_mode(sf: dict) -> str:
    ...
    if mode not in SF_MODES:
        raise ValueError(
            f"[sf] mode must be one of {SF_MODES}, got {mode!r}"
        )
    return mode


def render_cluster_toml(template: str, source: dict, sf_source: dict) -> str:
    """Substitute both binary sources into the template.

    Raises when a placeholder is missing or a table is empty — both would
    otherwise produce a cluster.toml that deploy.sh rejects much later
    with a fuzzier error.
    """
    for placeholder, table, name in (
        (SOURCE_PLACEHOLDER, source, "source"),
        (SF_SOURCE_PLACEHOLDER, sf_source, "sf_source"),
    ):
        if placeholder not in template:
            raise ValueError(f"template has no {placeholder} placeholder")
        if not table:
            raise ValueError(
                f"empty [{name}] table — need bin_path, project_path, or "
                f"github+rev"
            )
    rendered = template.replace(SOURCE_PLACEHOLDER, source_to_inline_toml(source))
    return rendered.replace(
        SF_SOURCE_PLACEHOLDER, source_to_inline_toml(sf_source)
    )
...
def main() -> None:
    ...
    sf_mode = validate_sf_mode(params.get("sf", {}))

    cluster_tpl = (SCRIPT_DIR / "cluster.toml.tpl").read_text()
    cluster = render_cluster_toml(
        cluster_tpl, params.get("source", {}), params.get("sf_source", {})
    )
    ...
    print(f"  source (legacy): {source_to_inline_toml(params['source'])}")
    print(f"  sf_source (SF):  {source_to_inline_toml(params['sf_source'])}")
    print(f"  sf mode:         {sf_mode}")
